...
finity.gs = {}
finity.theme = {
	main_container = Color3.fromRGB(150, 150, 150),
	separator_color = Color3.fromRGB(150, 150, 150),
...
function finity:addShadow(object, transparency)
	local shadow = self:Create("ImageLabel", {
		...
		ImageTransparency = transparency and true or .5,
...
function finity.new(isdark, gprojectName, thinProject)
	...
	if thinProject then
		thinMenu = thinProject
	end
	local toggled = true
	local typing = false
	local firstCategory = true
	local savedposition = UDim2.new(.5, 0, .5, 0)
	local finityData
	finityData = {
		UpConnection = nil,
	...
	self2.ChangeToggleKey = function(NewKey)
		...
		finityData.UpConnection = finity.gs["UserInputService"].InputEnded:Connect(function(Input)
			if Input.KeyCode == finityData.ToggleKey and not typing then
				...
				if toggled then
					pcall(self2.container.TweenPosition, self2.container, savedposition, "Out", "Sine", .5, true)
				else
					savedposition = self2.container.Position
					pcall(self2.container.TweenPosition, self2.container, UDim2.new(savedposition.Width.Scale, savedposition.Width.Offset, 1.5, 0), "Out", "Sine", .5, true)
				end
			end
		end)
	end
	self2.ChangeBackgroundImage = function(ImageID, Transparency)
		...
		if Transparency then
			...
			self2.container.ImageTransparency = .8
		end
	end
	finityData.UpConnection = finity.gs["UserInputService"].InputEnded:Connect(function(Input)
		if Input.KeyCode == finityData.ToggleKey and not typing then
			toggled = not toggled
			if toggled then
				self2.container:TweenPosition(UDim2.new(.5, 0, .5, 0), "Out", "Sine", .5, true)
			else
				self2.container:TweenPosition(UDim2.new(.5, 0, 1.5, 0), "Out", "Sine", .5, true)
			end
		end
	end)
	...
	self2.sidebar = self:Create("Frame", {
		Name = "Sidebar",
		BackgroundColor3 = Color3.new(.976471, .937255, 1),
		BackgroundTransparency = 1,
		BorderColor3 = Color3.new(.745098, .713726, .760784),
	...
	function self2:Category(name)
		...
		if firstCategory then
			finity.gs["TweenService"]:Create(category.hider, TweenInfo.new(.3), {
				BackgroundTransparency = 1
			}):Play()
			finity.gs["TweenService"]:Create(category.container, TweenInfo.new(.3), {
		...
		category.button.MouseEnter:Connect(function()
			finity.gs["TweenService"]:Create(category.button, TweenInfo.new(.2), {
			...
			for _, categoryf in next, self2.userinterface["Container"]["Categories"]:GetChildren() do
				if categoryf:IsA("ScrollingFrame") then
					if categoryf ~= category.container then
						finity.gs["TweenService"]:Create(categoryf.Hider, TweenInfo.new(.3), {
							BackgroundTransparency = 0
						}):Play()
						finity.gs["TweenService"]:Create(categoryf, TweenInfo.new(.3), {
							ScrollBarImageTransparency = 1
						}):Play()
					end
				end
			...
			function sector:Cheat(kind, name, callback, data)
				...
				if kind then
					if string.lower(kind) == "checkbox" or string.lower(kind) == "toggle" then
						...
						if data then
							if data.enabled then
								finity.gs["TweenService"]:Create(cheat.outerbox, TweenInfo.new(.2), {
									ImageColor3 = theme.checkbox_checked
								}):Play()
								finity.gs["TweenService"]:Create(cheat.checkboxbutton, TweenInfo.new(.2), {
									ImageColor3 = theme.checkbox_checked
								}):Play()
							end
						end
					...
					elseif string.lower(kind) == "color" or string.lower(kind) == "colorpicker" then
						...
						cheat.hsvbar.MouseButton1Down:Connect(function()
							...
							while uis:IsMouseButtonPressed("MouseButton1") do
								...
								finity.gs["TweenService"]:Create(cheat.arrowpreview, TweenInfo.new(.1), {
									Position = UDim2.new(position, 0, .5, -6)
								}):Play()
					...
					elseif string.lower(kind) == "dropdown" then
						...
						local function refreshOptions()
							...
							for _, value in next, options do
								...
								button.MouseEnter:Connect(function()
									finity.gs["TweenService"]:Create(button, TweenInfo.new(.1), {
										TextColor3 = theme.dropdown_text_hover
									}):Play()
								end)
								...
								finity.gs["TweenService"]:Create(button, TweenInfo.new(0), {
									TextTransparency = 1
								}):Play()
							end
							finity.gs["TweenService"]:Create(cheat.list, TweenInfo.new(0), {
								...
								Position = UDim2.new(0, 0, 1, 0),
								CanvasSize = UDim2.new(0, 0, 0, cheat.list["UIListLayout"].AbsoluteContentSize.Y),
								ScrollBarImageTransparency = 1,
								BackgroundTransparency = 1
							}):Play()
						end
							...
							if cheat.dropped then
								for _, button in next, cheat.list:GetChildren() do
									if button:IsA("TextButton") then
										finity.gs["TweenService"]:Create(button, TweenInfo.new(.2), {
											TextTransparency = 0
										}):Play()
									end
								end
								finity.gs["TweenService"]:Create(cheat.list, TweenInfo.new(.2), {
									Size = UDim2.new(1, 0, 0, math.clamp(cheat.list["UIListLayout"].AbsoluteContentSize.Y, 0, 150)),
									Position = UDim2.new(0, 0, 1, 0),
									ScrollBarImageTransparency = 0,
									BackgroundTransparency = .5
								}):Play()
						...
						cheat.dropdown.MouseEnter:Connect(function()
							finity.gs["TweenService"]:Create(cheat.selected, TweenInfo.new(.1), {
								TextColor3 = theme.dropdown_text_hover
							}):Play()
						end)
					...
					elseif string.lower(kind) == "textbox" then
						...
						cheat.background.MouseEnter:Connect(function()
							finity.gs["TweenService"]:Create(cheat.textbox, TweenInfo.new(.1), {
								TextColor3 = theme.textbox_text_hover
							}):Play()
						end)
						...
						cheat.textbox.Focused:Connect(function()
							typing = true
							finity.gs["TweenService"]:Create(cheat.background, TweenInfo.new(.2), {
								ImageColor3 = theme.textbox_background_hover
							}):Play()
						end)
					...
					elseif string.lower(kind) == "slider" then
						...
						if data.default then
							...
							finity.gs["TweenService"]:Create(cheat.visiframe, TweenInfo.new(.1), {
						...
						cheat.sliderbar.MouseButton1Down:Connect(function()
							...
							finity.gs["TweenService"]:Create(cheat.numbervalue, TweenInfo.new(.1), {
								Position = UDim2.new(size / 150, 5, .5, 0),
								TextTransparency = 0
							}):Play()
							moveconnection = mouse.Move:Connect(function()
								...
								local Position = UDim2.new(size / 150, 5, .5, 0)
								if Position.Width.Scale >= .6 then
									Position = UDim2.new(1, -cheat.numbervalue.TextBounds.X, .5, 10)
								end
								finity.gs["TweenService"]:Create(cheat.numbervalue, TweenInfo.new(.1), {
									Position = Position,
									TextTransparency = 0
					...
					elseif string.lower(kind) == "button" then
						...
						cheat.button.MouseEnter:Connect(function()
							finity.gs["TweenService"]:Create(cheat.background, TweenInfo.new(.2), {
								ImageColor3 = theme.button_background_hover
							}):Play()
						end)
						...
						cheat.button.MouseButton1Down:Connect(function()
							finity.gs["TweenService"]:Create(cheat.background, TweenInfo.new(.2), {
								ImageColor3 = theme.button_background_down
							}):Play()
						end)
